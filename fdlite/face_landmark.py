...
def face_detection_to_roi(face_detection: Detection, image_size: Tuple[int, int]) -> Rect:
    ...
    return bbox_to_roi(
        face_detection.bbox, image_size, rotation_keypoints=[left_eye, right_eye], scale=ROI_SCALE, size_mode=SizeMode.SQUARE_LONG
    )
...
class FaceLandmark:
    ...
    def __init__(self, model_path: Optional[str] = None) -> None:
        ...
        self.data_name = self.session.get_outputs()[0].name
        self.face_name = self.session.get_outputs()[1].name
    ...
    def __call__(self, image: Union[Image, np.ndarray, str], roi: Optional[Rect] = None) -> List[Landmark]:
        ...
        Returns:
            (list) List of face landmarks in normalized coordinates relative to
            the input image, i.e. values ranging from [0, 1].
        """
        height, width = self.input_shape[1:3]
        image_data = image_to_tensor(image, roi, output_size=(width, height), keep_aspect_ratio=False, output_range=(0.0, 1.0))
        ...
        return project_landmarks(
            raw_data, tensor_size=(width, height), image_size=image_data.original_size, padding=image_data.padding, roi=roi
        )
...
def face_landmarks_to_render_data(
    ...
    render_data = landmarks_to_render_data(
        face_landmarks, FACE_LANDMARK_CONNECTIONS, landmark_color, connection_color, thickness, normalized_positions=True, output=output
    )
    return render_data
